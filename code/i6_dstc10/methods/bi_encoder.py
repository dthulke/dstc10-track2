...
from i6_dstc10.methods.base import MethodWithDocumentDataset
from i6_dstc10.models.bi_encoder import BiEncoderModel, RobertaForBiEncoder
from i6_dstc10.preprocessing import process_input, process_knowledge, wrap_with_special_tokens
from i6_dstc10.utils import randrange_excluding
  ...
  def _sample_negative(self, query_item, document_type):
    if self.args.sample_document_uniform:
      positive_document_index = self._get_document_index(query_item['domain'], query_item['entity_id'],
                                                       query_item['doc_id'])
      sampled_document_index = randrange_excluding(0, len(self.documents_dataset), positive_document_index)
    ...
    if self.args.selection_level in ['domain_entity', 'entity']:
      ...
      negative_doc = random.choice(
        list(self.document_register[negative_domain][negative_entity].keys() - {query_item['doc_id']}))
    else:
  ...
  def __getitem__(self, index):
    ...
    if self.train_mode:
      if not self.args.sample_dialog_contexts or torch.rand(()).item() <= len(self.documents_dataset) / (len(self.query_dataset) + len(self.documents_dataset)):
        ...
        negative_document_index = self._sample_negative(query_item, document_type)
        ...
        query_input_ids = wrap_with_special_tokens(self.tokenizer,
                                                               self.documents_dataset[positive_document_index][
                                                                 'input_ids'])
        # Negative dialog
        negative_query_index = randrange_excluding(0, len(self.query_dataset), query_index)
        negative_document_input_ids = wrap_with_special_tokens(self.tokenizer, self.query_dataset[negative_query_index]['input_ids'])
...
class DataCollatorForBiEncoder:
  ...
  def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
    positive_document_input_ids = [f.get('positive_document_input_ids', None) for f in features]
    negative_document_input_ids = [f.get('negative_document_input_ids', None) for f in features]
    for f in features:
...
class BiEncoderMethod(MethodWithDocumentDataset):
  ...
  def preprocess_documents(self, features, add_special_tokens=False):
    ...
    if add_special_tokens:
      out['input_ids'] = wrap_with_special_tokens(self.tokenizer, out['input_ids'])
  ...
  def compute_metrics(self, p: EvalPrediction):
    ...
    for metric in self.metrics:
      results.update(
        metric.compute(predictions=prediction_ids, references=label_ids)
      )
    return results

  def postprocess_predictions(self, p: PredictionOutput, dataset):
    dataset.documents_dataset.load_faiss_index('embeddings', self.data_args.test_documents_faiss_index_path)
