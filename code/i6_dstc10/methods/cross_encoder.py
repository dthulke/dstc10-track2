...
from i6_dstc10.knowledge_utils import build_knowledge_document_register_with_city
...
from i6_dstc10.utils import iterate_values_in_nested_dict
...
class CrossEncoderDataset(Dataset):
  ...
  def _build_data_infos(self):
    ...
    for query_idx, query_item in enumerate(self.query_dataset):
      ...
      if self.args.selection_level in ["all", "domain", "domain_entity"]:
        data_info = list(iterate_values_in_nested_dict(self.document_register[city]))
      elif self.args.selection_level == 'entity':
        if isinstance(query_item['domain'], str):
          data_info = list(self.document_register[city][query_item['domain']].values())
        else:
          data_info_with_scores = [(doc, score) for domain, score in zip(query_item['domain'], query_item['score']) for doc in self.document_register[city][domain].values()]
          data_info, scores = zip(*data_info_with_scores)
          data_scores.extend(scores)
      elif self.args.selection_level == 'document':
        if isinstance(query_item['domain'], str):
          data_info = list(self.document_register[city][query_item['domain']][query_item['entity_id']].values())
        else:
  ...
  def _sample_negative(self, query_item, document_type):
    ...
    if negative_sample_level == "domain":
      if len(query_item['entity_candidates']) == 0:
        ...
        possible_domains = {'train', 'taxi'} | {c['domain'] for c in query_item['entity_candidates']}
    ...
    if self.args.selection_level in ['domain']:
      ...
      if len(query_item['entity_candidates']) == 0 or negative_domain in {'train', 'taxi'}:
        ...
        # TODO can we remove the hardcoding?
        possible_entities = {c['entity_id'] for c in query_item['entity_candidates'] if c['domain'] == negative_domain}
    ...
    if self.args.selection_level in ['domain_entity', 'entity']:
      ...
      possible_documents = list(self.document_register[city][negative_domain][negative_entity].keys() - {query_item['doc_id']})
      if len(possible_documents) == 0:
        negative_entity = random.choice(list(self.document_register[city][negative_domain].keys() - {query_item['entity_id']}))
        possible_documents = list(self.document_register[city][negative_domain][negative_entity].keys())
      negative_doc = random.choice(possible_documents)
    else:
      negative_doc = random.choice(list(self.document_register[city][negative_domain][negative_entity].keys()))
  ...
  def __getitem__(self, index):
    ...
    if not self.train_mode:
      ...
      city = self._get_city_from_source(query_item['source'])
      try:
        label_index = self._get_document_index(city, query_item['domain'], query_item['entity_id'], query_item['doc_id'])
        label = int(document_index == label_index)
      except Exception:
        label = 0
    else:
      # TODO do we somehow want to guarantee that the positives and negative samples are in the same batch?
      ...
      if document_type == 0:
        # Positive
        city = self._get_city_from_source(query_item['source'])
        document_index = self._get_document_index(city, query_item['domain'], query_item['entity_id'], query_item['doc_id'])
        label = 1
      else:
        document_index = self._sample_negative(query_item, document_type)
        label = 0

    input_ids = create_concatenated_dialog_knowledge_input(self.args, self.tokenizer, query_item['input_ids'], self.documents_dataset[document_index]['input_ids'])
...
class CrossEncoderMethod(Method):
      ...
      if 'nbest' in features['turns'][0][-1]:
        ...
        for i, turns in enumerate(features['turns']):
          for hyp in turns[-1]['nbest']:
            ...
            input_ids = process_input(self.model_args, current_turns, self.tokenizer)
  ...
  def compute_metrics(self, p: EvalPrediction):
    if self.config.num_labels == 2:
      ...
      prediction_ids = torch.sigmoid(torch.tensor(p.predictions))[:, 0] >= 0.5
    results = {}
  ...
  def postprocess_predictions(self, p: PredictionOutput, dataset):
    if self.config.num_labels == 2:
      ...
      scores = torch.nn.functional.logsigmoid(torch.tensor(p.predictions))[:, 0]
    else:
      assert False
    ...
    if self.model_args.hierarchical_selection_beam_search and len(dataset.data_scores) > 0:
      ...
      scores += np.log(np.array(dataset.data_scores)) * self.model_args.hierarchical_selection_beam_factor
    ...
    if self.model_args.process_all_in_nbest_last_turn and 'asr_score' in dataset.query_dataset.column_names:
      ...
      for query_idx in range(len(dataset.query_dataset)):
        ...
        list_of_asr_scores[-1].append(dataset.query_dataset['asr_score'][query_idx])
      # Combine nbest score
      if self.model_args.selection_nbest_combination_stratey == 'average':
        doc_scores = [torch.logsumexp(torch.stack(score_list), dim=0) - math.log(len(score_list)) for score_list in list_of_scores]
        ...
        for score_list, asr_scores in zip(list_of_scores, list_of_asr_scores):
          norm_asr_scores = torch.log_softmax(torch.tensor(asr_scores), 0)
          doc_scores.append(
            torch.logsumexp(norm_asr_scores.view(-1, 1) + torch.stack(score_list), dim=0)
          )
      else:
        assert False, f'Unsupported nbest detection combination strategy: {self.model_args.selection_nbest_combination_stratey}'
      # Get outputs
      return [
        get_label(start, *doc_score.topk(min(len(doc_score), self.model_args.selection_prediction_topk)))
  ...
  def _get_dataset(self, split, config_name=None):
    ...
    document_register = build_knowledge_document_register_with_city(document_dataset)
