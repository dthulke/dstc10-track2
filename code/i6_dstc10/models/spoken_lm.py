...
from transformers.generation_utils import BeamSearchEncoderDecoderOutput, GenerationMixin, BeamSearchOutput
...
class ShallowFusionConfig(PretrainedConfig):
    model_type = "shallow_fusion_lm"
    is_composition = True
...
class ShallowFusionModel(torch.nn.Module):
    def __init__(
        self,
        config,
        direct_model,
...
class PretrainedShallowFusionModel(PreTrainedModel):
    config_class = ShallowFusionConfig
    ...
    def __init__(
        ...
        self.model = ShallowFusionModel(config, direct_model, language_model)
...
class ShallowFusionModelForConditionalGeneration(PretrainedShallowFusionModel):

    def __init__(self, config, direct_model, language_model, scaling_factor=.3):
        super().__init__(config, direct_model, language_model)
    ...
    def forward(
        ...
        **unused,
        ...
        dm_encoder_outputs, lm_encoder_outputs = tuple(encoder_outputs.last_hidden_states)
        ...
        interpolated_scores = torch.mul(dm_scores, 1-self.scaling_factor) + torch.mul(lm_scores, self.scaling_factor)
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            for model, model_input_ids, attention_mask in zip(
                [self.direct_model, self.language_model],
                [input_ids, lm_input_ids],
                [dm_attention_mask, lm_attention_mask]
            ):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        return cls(config, direct_model, language_model, config.scaling_factor)
