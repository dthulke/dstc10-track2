...
class CreateShallowFusionCheckpointJob(Job):
  ...
  def _get_run_cmd(self):
      args = [
          "--dm_model_name_or_path", self.dm_model_name_or_path,
          "--lm_model_name_or_path", self.lm_model_name_or_path,
          "--scaling_factor", str(self.scaling_factor),
          "--checkpoint_path", self.out_model_path.get_path()
      ]
      run_cmd = [
          tk.uncached_path(gs.PYTHON_EXE),
          os.path.join(tk.uncached_path(self.code_root), "i6_dstc10/models/create_sf_checkpoint.py"),
          *args
      ]
      return run_cmd
  ...
  def hash(cls, kwargs):
      ...
      excluded_keys = ['mem_rqmt', 'cpu_rqmt', 'gpu_rqmt']
...
class CreateDensityRatioMethodCheckpoint(Job):
  ...
  def _get_run_cmd(self):
      args = [
          "--dm_model_name_or_path", self.dm_model_name_or_path,
          "--ilm_model_name_or_path", self.ilm_model_name_or_path,
          "--lm_model_name_or_path", self.lm_model_name_or_path,
      ...
      run_cmd = [
          tk.uncached_path(gs.PYTHON_EXE),
          os.path.join(tk.uncached_path(self.code_root), "i6_dstc10/models/create_dr_checkpoint.py"),
          *args
      ]
      return run_cmd
...
class CreateNoisyChannelCheckpointJob(Job):
  ...
  def _get_run_cmd(self):
      print(self.km_model_name_or_path)
      args = [
          "--dm_model_name_or_path", str(self.dm_model_name_or_path),
          "--cm_model_name_or_path", self.cm_model_name_or_path,
          "--lm_model_name_or_path", self.lm_model_name_or_path,
      ...
      run_cmd = [
          tk.uncached_path(gs.PYTHON_EXE),
          os.path.join(tk.uncached_path(self.code_root), "i6_dstc10/models/create_nc_checkpoint.py"),
          *args
      ]
      return run_cmd
